...
module.exports = function (client) {
    const description = {
        name: "jointocreatepubg",
        filename: "jointocreatepubg.js",
        version: "3.2"
    }
    new Promise(resolve => {
        setInterval(() => {
    ...
    client.on("voiceStateUpdate", (oldState, newState) => {
        ...
        if (!oldState.channelID && newState.channelID) {
            if (newState.channelID !== '1058424942514929734') return;
        ...
        if (oldState.channelID && newState.channelID) {
            ...
            if (oldState.channelID !== newState.channelID) {
                if (newState.channelID === '1058424942514929734')
    ...
    async function jointocreatechannel(user) {
        ...
        await user.guild.channels.create(`PUBG con ${user.member.user.username}`, {
            type: 'voice',
            parent: "849779939065593908",
            userLimit: 4
        }).then(async vc => {
